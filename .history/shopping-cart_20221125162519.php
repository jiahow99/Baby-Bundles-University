...
if(empty($_SESSION['products'])){
    $delete_cart = "DELETE FROM shopping_cart_table WHERE userid='$userid'";
    $db_controller->deleteQuery($delete_cart);
}

$select_cart = "SELECT * FROM shopping_cart_table WHERE userid='$userid'";

// Get products in the cart
$products = $db_controller->selectSingleProduct($select_cart);
...
if($num_rows > 0){
    ...
    if(isset($products)){
        foreach ($items_array as $product) {
            ...
            $product = $db_controller->selectSingleProduct($query);
    ...
    switch ($_GET['action']) {
        ...
        case 'remove':
            if(in_array($productid, $items_array)){
                ...
                $db_controller->updateCart($query);
        ...
        case 'placeorder':
            ...
            $delete_result = $db_controller->deleteQuery($empty_cart);
