...
if(isset($_POST['signup'])){
  ...
  if( $same_username > 0 ){
    ...
    $username_err = array_push($username_err, "Only letters and digits are allowed and ATLEAST 4 characters");
  ...
  if( empty($errormsg) ){
    ...
    if(!$result){
      ...
      // Get shopping cart
      $userid = $db_controller->conn->lastInsertId();
      $cart = $db_controller->selectUser("SELECT * FROM shopping_cart_table WHERE userid='$userid'");
      ...
      setcookie('userid', $db_controller->conn->lastInsertId() , time()+604800);
      setcookie('is_admin', '0', time()+604800);
      setcookie('quantity', count($_SESSION['products']) , time()+604800);
