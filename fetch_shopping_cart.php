...
// Get all product
$result = $dbcontroller->selectUser("SELECT * FROM shopping_cart_table WHERE userid='$userid'");
...
foreach( $item_array as $product ){
    $item = $dbcontroller->selectUser("SELECT * FROM product_table WHERE product_id='$product'");
    $product_array = array(
